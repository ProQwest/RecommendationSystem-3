...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
           ...
           // double cs = new CosineVectorSimilarity().similarity(userVector, iv);
            double cs = userVector.dot(iv)/(userVector.norm()*iv.norm());
            output.set(e.getKey(), cs);
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            ...
            double meanRate = p.getItemId()/userRatings.size();
            double wt = p.getValue()- meanRate;
            
           // if (p != null && p.getValue() >= 3.5) {
                // The user likes this item!
                // TODO Get the item's vector and add it to the user's profile ->Done
            	SparseVector v = model.getItemVector(p.getItemId());
            	MutableSparseVector mv = MutableSparseVector.create();
            	mv = v.mutableCopy();
            	mv.multiply(wt);
            	for(long keyValue: mv.keySet())
            	{
            		profile.set(keyValue, mv.get(keyValue));
            	}
