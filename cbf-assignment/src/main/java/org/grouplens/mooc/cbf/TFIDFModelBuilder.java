...
//        MutableSparseVector idocFreq = MutableSparseVector.create(tagIds.values());
//        idocFreq.fill(0);
        ...
        // Create a map to store the item TF v	ectors.
        ...
        for (long item: items) {
            ...
            // TODO Populate the work vector with the number of times each tag is applied to this item. -> Done
            Long id=(long) 0; //initialize
            for(String s: dao.getItemTags(item))
        	{
            	try
            	{
            ...
            // TODO Increment the document frequency vector once for each unique tag on the item. -> Done
            for(VectorEntry e: work.fast())
            {
            	docFreq.add(e.getKey(),1.0);
            }
        ...
        int noOfDoc = items.size();
        for (VectorEntry e: docFreq.fast()) {
            // TODO Update this document frequency entry to be a log-IDF value -> Done
        	double idf = Math.log(noOfDoc/e.getValue());
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
            ...
            //tv.multiply(docFreq);
            for (long key: tv.keySet())
            {
            	double tf_idf = tv.get(key) * docFreq.get(key);
            	tv.set(key, tf_idf);
            }
            ...
            for (long key: tv.keySet())
            {
            	double tf_idf_norm = tv.get(key)/norm;
            	tv.set(key, tf_idf_norm);
            }
